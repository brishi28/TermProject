...
import sys
from tkinter import *
from pathPlanningFunctional import Cell, PathPlan
import threading
...
class cozmoRobot(object):
    ...
    def moveASpace(self):
        ...
        def avoidCube(self, cubeNumber):
            ...
            self.safeSpaces.add(self.cubes[cubeNumber-1])
    ...
    def mapSpace(self, info):
        print(self.safeSpaces)
        if info == 'safe':
            self.safeSpaces.add((self.currentX, self.currentY))
...
def startPathing(robot: cozmo.robot.Robot):
    ...
    if currentState == 'pathing':
        ...
        x, y = (0, 3)
        pathing = PathPlan()
        pathing.initMap(coz.maxX, coz.maxY, coz.safeSpaces, (coz.currentX, coz.currentY), (x, y))
